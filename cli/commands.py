...
def check_stale(threshold_hours):
    ...
    try:
        ...
        stale_plans = session.query(Plan).filter(Plan.last_synced_at < threshold).all()
            ...
            stale_data.append(
                {
                    ...
                    "name": plan.name,
                    "last_synced_at": plan.last_synced_at.isoformat()
                    if plan.last_synced_at
                    else None,
                    "hours_stale": int(
                        (datetime.now(UTC) - plan.last_synced_at).total_seconds() / 3600
                    )
                    if plan.last_synced_at
                    else None,
                }
