...
class FulfillOrderService:
    ...
    def __init__(self, tm_adapter: TicketmasterAdapter):
        self._tm = tm_adapter

    def fulfill(self, order: Order, plan: Plan) -> list[dict]:
        """
        ...
        acceptance = self._tm.accept_offer(
            tm_order["offer_id"], idempotency_key=accept_idempotency_key, seat_ids=seat_ids
        )
        ...
        order.add_domain_event(OrderConfirmed(order_id=order.order_id, tickets=tickets))
