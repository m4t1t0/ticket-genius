...
def encode_cursor(last_seen_id: UUID, last_seen_sort_key: str) -> str:
    ...
    data = {
        "id": str(last_seen_id),
        "sort_key": last_seen_sort_key,
    }
    json_str = json.dumps(data, separators=(",", ":"))
    return base64.urlsafe_b64encode(json_str.encode()).decode()


def decode_cursor(cursor: str) -> Optional[Tuple[UUID, str]]:
    """
...
def apply_cursor_filter(query, cursor: Optional[str], id_column, sort_column) -> tuple:
    """
...
def get_next_cursor(results: list, id_attr: str, sort_attr: str) -> Optional[str]:
    """
